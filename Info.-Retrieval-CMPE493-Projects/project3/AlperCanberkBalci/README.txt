ReadMe file for Cmpe493 Project3 2021 Fall Bogazici University

Multinomial Naive Bayes (NB) and k Nearest Neighbor (kNN) algorithms for text classification. 

Python version = 3.8

1.1- Only module, that is, the main.py file.
1.2- User must put the SGML files into the same directory with main.py
1.3- One can simply run it by writing to the terminal main.py, and it would start running.
1.4- While it runs, it reads the data and do the indexing, all the processes.

On the console there will be reports.
